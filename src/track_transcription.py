...
class Track:
  ...
  def __init__(self, output_dir, override_results=False):
    ...
    #Create the thread pool that will run registration and transformation
...
#####PUBLIC CLASS FUNCTIONS
  ...
  def set_image_information(self, images_dir, frame_prefix, frame_suffix):
    ...
    if self.max_frame_id <= self.min_frame_id:
      raise Exception ("The maximum frame id {0} can not be less than the minimum id {1}"\
        .format(self.max_frame_id, self.min_frame_id))
  ...
  def register_frames(self, reference_list, moving_list, registration_params):
    ...
    for registration in range(len(work_list)):
      async_result = self.pool.apply_async(self.register, work_list[registration])
...
#      transformed_image = destination
  ...
  def track(self, np_array, memory, distance):
    ...
    #Propagate the position of the tracked particles to all frames.
    #if merged_df != None:
    

  def propagate_tracked_results(self, tracked_particles):
    """Propagate the (x,y) position of the trakced particles to all frames.
    For every frame where the particle does not exist, the (x,y) position will
    be approximated using registration and transformation with the closest
    frame where the point exist, or the closest pilot frame. 

    Parameters:
      tracked_particles: A dataframe generated by track py with the tracked ids. 

    """

    #Find the number of particles
    particles_ids =  tracked_particles.particle.unique().tolist()
    n_particles = len(particles_ids)

    #Generate a list of pilot frames
    if self.destination == None:
      raise Exception("The destination frame was not set.")

    if self.stride == None:
      raise Exception("The iterative registraiton stride was not set.")
    ...
    for my_particle in particles_ids:
      ...
      print("Propagating tracks for particle:{0}".format(my_particle))
      #Create a new dataframe for every particle.
      self.particle_df = tracked_particles[tracked_particles.particle == my_particle]


      #Create a set of original frames and pilot frames
      #anchor_frames = set(existing_frames, pilot_frames)  

      #Make the frame column the new index of the particle dataframe
      self.particle_df = self.particle_df.set_index('frame')

      original_frames = self.particle_df.index

      #print "Original frames for particle {0} are:{1}".format(my_particle, original_frames)

      for frame in range (self.min_frame_id, self.max_frame_id+1):
  ...
  def transform_tracked_point(self, frame, pilot_frames, original_frames):
    ...
    #if the particle is already transformed to a given frame, do nothing
    if frame in self.particle_df.index:
      return
    
    if frame < self.min_frame_id:
      raise Exception("Trying to transform the point to an invalid frame index:{0}".format(frame))

    if frame > self.max_frame_id:
      raise Exception("Trying to transform the point to a non-existing frame index:{0}".format(frame))

    #Check the direction of the closest original point.
    distance_to_original = [abs(x-frame) for x in original_frames]
    closest_original_frame_index  = np.argmin(distance_to_original)
    closest_original_frame =  original_frames[closest_original_frame_index] 

    #print "Closest original frame to {0} is {1}".format(frame, closest_original_frame)
    if abs(frame - closest_original_frame) > self.stride:
      #Then I have to find the closest pilot.

      if closest_original_frame >  frame:
        #Look at the closest upper pilot 
        upper_pilots = sorted( [x for x in pilot_frames if x > frame])
        #print"Looking into the first upper pilot: {0}".format(upper_pilots)
        reference_frame = upper_pilots[0]
      else:
        #Look at the closest lower pilot 
        lower_pilots = sorted([x for x in pilot_frames if x < frame])
        #print lower_pilots
        #print "Looking into the last lower pilots: {0}".format(lower_pilots)
        reference_frame = lower_pilots[-1]

    else:      
      reference_frame = closest_original_frame

    if reference_frame == frame:
      raise Exception ("Error, the closest pilot frame is equal to the current frame{0}".format(frame))

    if not reference_frame in self.particle_df.index:
      self.transform_tracked_point(reference_frame, pilot_frames, original_frames)

    print "Registering frame {0} with reference {1} ".format(frame, reference_frame)

    #Transform the point from the reference frame. 
    x_position = self.particle_df.loc[reference_frame].x_index
    y_position = self.particle_df.loc[reference_frame].y_index
    point_to_transform = [x_position, y_position]

    #register the frame to the reference frame.
    ref_id = self.id2str(reference_frame)
    flt_id = self.id2str(frame)
    self.register_frames([ref_id], [flt_id],self.registration_params)
    registration_result = self.get_registration_results(ref_id, flt_id)

    #Transform the (x,y) position
    input_files_dir = os.path.join(self.transformix_dir, "general_inputs") 
    self.create_dir(input_files_dir)
    frame_input_file = os.path.join(input_files_dir,"input_points" + ref_id +\
      "-" + flt_id + ".txt")
    self.points_to_transformix([point_to_transform], frame_input_file)

    #Create the transformix directory
    transform_directory = os.path.join(self.transformix_dir, "general_transformix_run")
    self.create_dir(transform_directory, override=True)
    ...
    if len(output_points) != 1:
        raise Exception ("ERROR: The number of transformed points does not equal 1 in file:{0}"\
          .format(output_points_file))

    trans_point = output_points[0]
    #Add the new row to self.particle_df
    new_row = DataFrame(columns= self.particle_df.columns, index = [frame])
    new_row.set_value(frame, 'x_index', trans_point[0])
    new_row.set_value(frame, 'y_index', trans_point[1])

    self.particle_df = self.particle_df.append(new_row)
    #print self.particle_df.index
...
###PRIVATE CLASS FUNCTIONS   
  ...
  def overlay_tracking_results(self, destination_dir, tracking_results, alpha_in = 0.90, source_dir=None):
